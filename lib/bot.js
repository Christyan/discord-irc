...
class Bot {
  constructor(options) {
    ...
    this.bufferingMessage = options.bufferingMessage;
    this.writeAsAnsi = options.writeAsAnsi || false;
    ...
    this.excludeNickFromMessage = options.excludeNickFromMessage || "";

    // buffering
    this.bufferedMessage = new Map();
  ...
  sendToDiscord(author, channel, text) {
    ...
    const withFormat = formatFromIRCToDiscord(text, this.writeAsAnsi);
    ...
    var withAuthor = ""
    if (this.excludeNickFromMessage == patternMap.author)
      withAuthor = patternMap.withMentions;
    else
      withAuthor = Bot.substitutePattern(this.formatDiscord, patternMap);
    logger.debug('Sending message to Discord', withAuthor, channel, '->', `#${discordChannel.name}`);
    if (this.bufferingMessage)
    {
      this.bufferedMessage.set(discordChannel, (this.bufferedMessage.has(discordChannel) ? this.bufferedMessage.get(discordChannel) : "") + withAuthor + "\n");
    }
    else
        discordChannel.send(withAuthor);
  }

  DelayedMessageSend(clientdata) {
    if (clientdata.bufferedMessage == null || clientdata.bufferedMessage.size == 0) {
        setTimeout(clientdata.DelayedMessageSend, 1000, clientdata);
        return;
    }
    var _bufferedMessage = new Map();
    _bufferedMessage = clientdata.bufferedMessage;
    clientdata.bufferedMessage = new Map();
    
    for (var [key, value] of _bufferedMessage) {
      var array = value.match(/(.|[\r\n]){1,1744}\n/g);
      for (var element of array) {
        if (clientdata.writeAsAnsi)
        {
          key.send("```ansi\n" + element + "\n```");
        }
        else
